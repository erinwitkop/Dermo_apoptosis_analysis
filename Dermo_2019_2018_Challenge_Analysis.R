...
MOI_1hr_2019_APOP_join_Agranular_Granular <- MOI_1hr_2019_APOP_join %>% filter(Gate =="Q1-UL" | Gate =="Q1-UR" | Gate =="Q1-LL" | Gate =="Q1-LR"|
...
Dermo_Inhibitor_2020_PHAGO_join_phago_combined_plot <- ggplot(Dermo_Inhibitor_2020_PHAGO_join_phago_combined, aes(x=Gate, y = revised_percent_of_this_plot, fill = Treat)) + 
         geom_boxplot() +
           ...
           theme(legend.text = element_text(size=12)) + 
           scale_y_continuous(labels = function(x) paste0(x, "%"), limits=c(0,100)) +
           scale_fill_manual(name="Treatment", labels=c("BEADS and LPS ", "Perkinsus marinus"), values=c("#aa4dce","#c89832")) +
...
# Perkinsus is phagocytosed significantly more than the BEADS+LPS immune activated treatment
